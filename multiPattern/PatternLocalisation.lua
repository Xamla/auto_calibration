...
function PatternLocalisation:plotCrosshair(img, posX, posY, crossSize)
  ...
  cv.line{img, {(centerX)-2, (centerY)},
                 {(centerX)-2-lineLength, (centerY)}, {0,0,0}}
  cv.line{img, {(centerX)+2, (centerY)},
                 {(centerX)+2+lineLength, (centerY)}, {0,0,0}}

  cv.line{img, {(centerX), (centerY)-2},
                 {(centerX), (centerY)-2-lineLength} , {0,0,0}}
  cv.line{img, {(centerX), (centerY)+2},
                 {(centerX), (centerY)+2+lineLength} , {0,0,0}}


  for i=-1,1,2 do
    cv.line{img, {(centerX)-4, (centerY)+i},
                   {(centerX)-2-lineLength, (centerY)+i}, {250,200,200}}
    cv.line{img, {(centerX)+4, (centerY)+i},
                   {(centerX)+2+lineLength, (centerY)+i}, {250,200,200}}

    cv.line{img, {(centerX)+i, (centerY)-4},
                   {(centerX)+i, (centerY)-2-lineLength} , {250,200,200}}
    cv.line{img, {(centerX)+i, (centerY)+4},
                   {(centerX)+i, (centerY)+2+lineLength} , {250,200,200}}
  end
end
...
function PatternLocalisation:mirrorPatternPoints(points, patWidth, patHeight, imgDebug)
  ...
  for col=0, patHeight-1 do
    for row=0, patWidth-1 do
      pointsResorted[col*patWidth+row+1][1][{}] = points[(patWidth)*(patHeight-col)-patWidth + row + 1][1][{}]
    end
  end
...
function PatternLocalisation:calcCamPose(inputImg, camIntrinsics, patternData, doDebug, imgShowInput)
  ...
  local found, points = cv.findCirclesGrid{ image = inputImg, patternSize = { height = patternData.height, width = patternData.width }, flags = cv.CALIB_CB_ASYMMETRIC_GRID+cv.CALIB_CB_CLUSTERING, blobDetector=blobDetector}

  if found then
    local points3d = self:generatePatternPoints(patternData.width, patternData.height, patternData.pointDist)
    local poseFound, poseCamRotVector, poseCamTrans=cv.solvePnP{objectPoints=points3d, imagePoints=points, cameraMatrix=camIntrinsics, distCoeffs=torch.DoubleTensor(1,5):zero()}
  ...
  if doDebug then
    ...
    cv.putText{imgShow, text="press any key to continue", org={x=30, y=30}, fontFace=cv.FONT_HERSHEY_SIMPLEX,
...
function PatternLocalisation:calcCamPoseMirrored(pointsResorted)
  ...
  local poseFound, poseCamRotVector, poseCamTrans=cv.solvePnP{objectPoints=points3d, imagePoints=pointsResorted,
...
function PatternLocalisation:calcCamPoseROI(inputImg, rectROI, doDebug)
  ...
  imgROI[{{rectROI[1][2], rectROI[2][2]}, {rectROI[1][1], rectROI[2][1]}}] = inputImg[{{rectROI[1][2], rectROI[2][2]}, {rectROI[1][1], rectROI[2][1]}}]
...
function PatternLocalisation:findCircles(inputImg, doDebug)
  ...
  for i=1,resultsTmp.size do
    table.insert(results, {radius=resultsTmp.data[i].size/2.0,
  ...
  if doDebug == true then
    ...
    for key,val in ipairs(results) do
       cv.circle{img = imgScale, center={x=val.pos[1]*rescaleFactor*circleScale, y=val.pos[2]*rescaleFactor*circleScale},
                 radius = val.radius*rescaleFactor*circleScale,
                 color = {0,255,255}, thickness=2, lineType=cv.LINE_AA, shift=shiftBits}
    end
...
function PatternLocalisation:expandCluster(kdtree, circleCenters, eps, minNPoints, currPointIndex, neighbours, pointStatus, cluster)
  ...
  while i<= loopEnd do
    ...
    if bit.band(pointStatus[index], self.clusterStatus.kProcessed) == 0 then
      ...
      local tmp  -- not used
      tmp, indices, dists = kdtree:radiusSearch{query=queryPoint, radius=eps*eps, maxResults=minNPoints,
...
function PatternLocalisation:espChecking(circleCentersInput, queryX, queryY, debugImg)
  ...
  cv.circle{img = debugImg, center={x=queryPoint[1][1]*rescaleFactor*circleScale, y=queryPoint[1][2]*rescaleFactor*circleScale},
           radius = 5*circleScale,
           color = {50,50,255}, thickness=3, lineType=cv.LINE_AA, shift=shiftBits}
  ...
  for i=1, nNeighbours do
    ...
    cv.circle{img = debugImg, center=center, radius = radius,
              color = {255,0,255}, thickness=1, lineType=cv.LINE_AA, shift=shiftBits}
  end
...
function PatternLocalisation:getPatternId(imgInput, points, pattern)
  ...
  local darkColor = (self:getPatPointCenterColor(imgGray, idRef[1]) + self:getPatPointCenterColor(imgGray, idRef[2])) / 2.0
  local lightColor = (self:getPatPointCenterColor(imgGray, idRef[3]) + self:getPatPointCenterColor(imgGray, idRef[4])
                        +self:getPatPointCenterColor(imgGray, idRef[5])) / 3.0
...
function PatternLocalisation:processImg(inputImg)
    ...
    if #cluster >= nPatternPoints and #cluster < nPatternPoints*2 then  -- process only clusters which have at least as many points as the searched pattern and not more then two times the number of pattern points
      ...
      if self.debugParams.clustering == true then
        ...
        for j, pointIdx in ipairs(cluster) do
          ...
          cv.circle{img = imgHulls, center=center, radius = radius,
                   color = self.colorTab[colorNum], thickness=2*1.0/self.imgShowRescale, lineType=cv.LINE_AA, shift=shiftBits}
        end
        ...
        for j=1, hull:size(1)-1 do
          cv.line{img=imgHulls, pt1={hull[j][1], hull[j][2]}, pt2={hull[j+1][1], hull[j+1][2]},
                  color=self.colorTab[colorNum], thickness=2*1.0/self.imgShowRescale, lineType=cv.LINE_AA}
        end

        cv.line{img=imgHulls, pt1={hull[hull:size(1)][1], hull[hull:size(1)][2]}, pt2={hull[1][1], hull[1][2]},
                color=self.colorTab[colorNum], thickness=2*1.0/self.imgShowRescale, lineType=cv.LINE_AA}
  ...
  for i, hull in ipairs(hullList) do
     ...
     if self.debugParams.clustering == true then
        cv.putText{imgHulls, text=""..i, org={x=hull[hull:size(1)][1], y=hull[hull:size(1)][2]}, fontFace=cv.FONT_HERSHEY_SIMPLEX,
                   fontScale=0.7*1.0/self.imgShowRescale, color={70, 70, 70}, thickness=3}
     end
    ...
    if self.camIntrinsics ~= nil then
      ...
      local blobDetector = cv.SimpleBlobDetector{self.circleFinderParams}
      patternFound, pointsSorted = cv.findCirclesGrid{ image = imgMasked, patternSize = { height = self.pattern.height, width = self.pattern.width }, flags = cv.CALIB_CB_ASYMMETRIC_GRID+cv.CALIB_CB_CLUSTERING, blobDetector=blobDetector}
    end
      ...
      if id>0 then
        ...
        if self.debugParams.pose == true then
           ...
           if pointsSorted ~= nil then
              ...
              cv.putText{imgHulls, text=""..id, org={x=pointsSorted[1][1][1]+8, y=pointsSorted[1][1][2]+8},
                         fontFace=cv.FONT_HERSHEY_SIMPLEX, fontScale=0.8*1.0/self.imgShowRescale, color={80, 80, 255}, thickness=3}
              print("ID "..id)
              if camPose ~= nil then
                print("cam pose: ")
                print(camPose)
        ...
        if self.debugParams.pose == true then
           ...
           if pointsSorted ~= nil then
              ...
              cv.putText{imgHulls, text=""..noIdPat, org={x=pointsSorted[1][1][1]+8, y=pointsSorted[1][1][2]+8},
                         fontFace=cv.FONT_HERSHEY_SIMPLEX, fontScale=0.6*1.0/self.imgShowRescale, color={0, 0, 255}, thickness=1}
              print(camPose)
           end
